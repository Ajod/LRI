...
def getMongoAuth(user=None,
                 hostname="localhost",
                 passwordsPath=homeDir() + "/.ssh/mongo-passwords.json",
                 logger=None,
                 verbose=True):
    """
...
#     if mongoServer == "datascience01" and not isHostname("datascience01"):
    ...
    passwords = None
    if isFile(passwordsPath):
        passwords = jsonFileToObject(passwordsPath)
    else:
        logError(passwordsPath + " not found, a localhost mongodb auth will be used...",
